...
class HGT(nn.Module):
    def __init__(self, conf, data):
        ...
        self.LiT_bi_mask = LiT(     ####?
            conf=LiT_bi_conf,
            data={
                "sp_graph": self.data["graph_bi_mask"],
            }
        )
        ...
        self.Agg_bi_mask = LGCN(
            conf=Agg_bi_conf, data={
                "graph": self.data["graph_bi_mask"]
            }
        )
    ...
    def forward(self,test):   ####
        ...
        for layer in range(self.conf["n_trans_layer"]):
            ...
            if test:
                b_i2b, i_b2i = self.Agg_bi_mask(b_feature, i_feature, layer=layer)  ####
            else:
                b_i2b, i_b2i = self.Agg_bi(b_feature, i_feature, layer=layer)

            i_feature = i_feature_cf + i_b2i   #这句在图中没有直观体现 公式4 5


            if test:
                b_feature_T = self.LiT_bi_mask(
                    i_feature, b_i2b, self.conf["bi_alpha"], self.eval_bundles, layer=layer).squeeze(0)

            else:
                b_feature_T = self.LiT_bi(
                    i_feature, b_i2b, self.conf["bi_alpha"], self.eval_bundles, layer=layer).squeeze(0)  #6 7 8 9式


            b_feature = b_feature_T + b_u2b  #10式


            u_feature_T = self.LiT_ub(
                b_feature, u_b2u, self.conf["ub_alpha"], self.eval_users, layer=layer).squeeze(0)  #11 12 13 14

            u_feature = u_feature_T + u_feature_cf  #15
