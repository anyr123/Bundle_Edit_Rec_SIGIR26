...
class BasicDataset(Dataset):
    ...
    def load_B_I_affiliation(self,train=True,rho=0):
        # with open(os.path.join(self.path, self.name, 'bundle_item.txt'), 'r') as f:
        #     return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
        if train:
            print('load train B-I')
            with open(os.path.join(self.path, self.name, 'BPRMF_bundle_item_mask0.5.txt'), 'r') as f:
                return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))

        else:
            mask  = RHO_MASK_MAP.get(rho, '0.5')
            fname = f'BPRMF_bundle_item_mask{mask}.txt'
            print(f'load test B-I  [rho={rho} → {fname}]')
            with open(os.path.join(self.path, self.name, fname), 'r') as f:
                return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')),
                                f.readlines()))
...
class AssistDataset(BasicDataset):
    def __init__(self, path, name, sample=None,rho=0):
        ...
        # B-I test
        self.B_I_pairs = self.load_B_I_affiliation(train=False,rho=rho)
        indice = np.array(self.B_I_pairs, dtype=np.int32)
        values = np.ones(len(self.B_I_pairs), dtype=np.float32)
        self.ground_truth_b_i_test = sp.coo_matrix(
            (values, (indice[:, 0], indice[:, 1])), shape=(self.num_bundles, self.num_items)).tocsr()

        print_statistics(self.ground_truth_b_i_test, 'B-I statistics test')
...
def get_dataset(args, negative_num):
    ...
    sample = args.sample
    # 加载bundle-item的归属信息
    assist_data = AssistDataset(path, name, rho=args.rho)
