...
class BRECModel_Distance(BaseModel):
    ...
    def __init__(self, args, info, graph):
        ...
        # self.dc = FermiDiracDecoder(r=args.r, t=args.t)
        ub_graph, ui_graph, bi_graph,bi_graph_test= graph
        #  deal with weights
        ...
        #  pooling graph test
        bundle_size_test = bi_graph_test.sum(axis=1) + self.epison
        # bi_graph_test每一行也都除了 bundle所含item的数量
        bi_graph_test = sp.diags(1 / bundle_size_test.A.ravel()) @ bi_graph_test   ####


        self.pooling_graph = to_tensor(bi_graph).to(args.device)
        self.pooling_graph_test = to_tensor(bi_graph_test).to(args.device)####
    ...
    def propagate(self,test=False):
        ...
        if test:  ####
            itemLevel_bundles_feature = F.normalize(torch.matmul(self.pooling_graph_test, itemLevel_items_feature_tangent))
        else:
