...
def main(args):
    ...
    for lr, dropout, dim, negative_num, num_layers, manifold,itemLevelCoef,itemLevel_c,bundleLevel_c,temperature in product(args.lr, args.dropout, args.dim,
        ...
        # model.load_state_dict(torch.load('./hyper_youshu'), strict=False)
        ...
        while retry >= 0:
            ...
            for epoch in range(args.epochs):
                ...
                if epoch % args.eval_freq == 0:
                    print('Val:')
                    output_metrics1 = test(model, val_loader, args, metrics_val,metrics_val_itemLevel,metrics_val_bundleLevel, epoch,test=False)
                    print('Test:')
                    output_metrics2 = test(model, test_loader, args, metrics_test,metrics_test_itemLevel,metrics_test_bundleLevel, epoch,test=True)
                    # logging.info()
