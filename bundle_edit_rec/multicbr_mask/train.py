...
def main():
    ...
    for lr, l2_reg, UB_ratio, UI_ratio, BI_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        # model.load_state_dict(torch.load('./Youshu'), strict=False)
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if (batch_anchor + 1) % test_interval_bs == 0:
                    metrics = {}
                    metrics["val"] = test(model, dataset.val_loader, conf,bi_conf='val')
                    metrics["test"] = test(model, dataset.test_loader, conf,bi_conf='test')
...
def test(model, dataloader, conf,bi_conf):
    tmp_metrics = {}
    ...
    device = conf["device"]
    model.eval()
    rs = model.get_multi_modal_representations(bi_conf,test=True)
