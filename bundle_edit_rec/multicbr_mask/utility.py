...
class Datasets():
    def __init__(self, conf):
        ...
        b_i_pairs, b_i_mask_graph = self.get_bi_test()
        ...
        self.graphs = [u_b_graph_train, u_i_graph, b_i_graph,b_i_mask_graph]
    ...
    def get_bi(self):   #### train_bi
        with open(os.path.join(self.path, self.name, 'BPRMF_bundle_item_mask0.5.txt'), 'r') as f:
        #with open(os.path.join( './Fold_bundle_item_mask0.5.txt'), 'r') as f:
    ...
    def get_bi_test(self):
            ...
            with open(os.path.join(self.path, self.name, fname), 'r') as f:
                b_i_pairs = list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')),
                                    f.readlines()))
