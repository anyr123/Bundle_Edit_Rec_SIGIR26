...
class MultiCBR(nn.Module):
    def __init__(self, conf, raw_graph):
        ...
        self.ub_graph, self.ui_graph, self.bi_graph,self.bi_test_graph = raw_graph
        ...
        self.BI_propagation_graph_val = self.get_propagation_graph(self.bi_graph)    ####
        self.BI_aggregation_graph_val = self.get_aggregation_graph(self.bi_graph)    ####


        self.BI_propagation_graph_test = self.get_propagation_graph(self.bi_test_graph)
        self.BI_aggregation_graph_test = self.get_aggregation_graph(self.bi_test_graph)
    ...
    def get_multi_modal_representations(self,bi_conf='train',test=False):
        ...
        if test and bi_conf== 'val':
            ...
            UI_bundles_feature = self.aggregate(self.BI_aggregation_graph_val, UI_items_feature, "BI", test)
        elif test and bi_conf== 'test':
            UI_users_feature, UI_items_feature = self.propagate(self.UI_propagation_graph_ori, self.users_feature, self.items_feature, "UI", self.UI_layer_coefs, test)
            UI_bundles_feature = self.aggregate(self.BI_aggregation_graph_test, UI_items_feature, "BI", test)
        else:
        ...
        if test and bi_conf == 'val':
            BI_bundles_feature, BI_items_feature = self.propagate(self.BI_propagation_graph_val, self.bundles_feature, self.items_feature, "BI", self.BI_layer_coefs, test)
            BI_users_feature = self.aggregate(self.UI_aggregation_graph_ori, BI_items_feature, "UI", test)
        elif test and bi_conf == 'test':
            BI_bundles_feature, BI_items_feature = self.propagate(self.BI_propagation_graph_test, self.bundles_feature, self.items_feature, "BI", self.BI_layer_coefs, test)
            BI_users_feature = self.aggregate(self.UI_aggregation_graph_ori, BI_items_feature, "UI", test)
        else:
