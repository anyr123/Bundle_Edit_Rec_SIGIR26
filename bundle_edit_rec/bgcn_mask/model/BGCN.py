...
class BGCN(Model):
    ...
    def __init__(self, info, dataset, raw_graph, device, pretrain=None):
        ...
        ub_graph, ui_graph, bi_graph,bi_graph_test = raw_graph
        ...
        bundle_size_test = bi_graph_test.sum(axis=1) + 1e-8
        ...
        self.pooling_graph_test = to_tensor(bi_graph_test).to(device)
        print('finish generating pooling test graph')
    ...
    def propagate(self,test=False):
        ...
        if test:
            atom_bundles_feature = F.normalize(torch.matmul(self.pooling_graph_test, atom_items_feature))  ####
        else:
