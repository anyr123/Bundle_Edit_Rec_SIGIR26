...
def main():
    ...
    for lr, decay, message_dropout, node_dropout \
        ...
        # model.load_state_dict(torch.load('./midgn_youshu'), strict=False)
            ...
            if retry >=0:
                ...
                for epoch in range(CONFIG['epochs']):
                    ...
                    if epoch % CONFIG['test_interval'] == 0:
                        output_metrics = test(model, eval_loader, device, CONFIG, metrics,test=False)   ###val
                        test_metrics = test(model, test_loader, device, CONFIG, test_metrics,test=True)   ####test
