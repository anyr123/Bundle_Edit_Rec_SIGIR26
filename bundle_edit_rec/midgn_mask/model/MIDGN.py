...
class MIDGN(Model):
    ...
    def __init__(self, info, dataset, raw_graph, device, pretrain=None):
        ...
        if bi_graph_test.shape == (self.num_bundles, self.num_items):  #########################33
            tmp = bi_graph_test.tocoo()
            self.bi_graph_test_h = list(tmp.row)
            self.bi_graph_test_t = list(tmp.col)
            self.bi_graph_test_shape = bi_graph_test.shape
        else:
            raise ValueError(r"raw_graph's shape is wrong")
    ...
    def propagate(self,test=False):
        ...
        if test:
            atom_bundles_feature, atom_item_feature, self.bi_avalues = self._create_star_routing_embed_with_p(
                self.bi_graph_test_h,
                self.bi_graph_test_t,
                self.bundles_feature,
                self.items_feature,
                self.num_bundles,
                self.num_items,
                self.bi_graph_test_shape,
                n_factors=1,
                pick_=False)
        else:
            atom_bundles_feature, atom_item_feature, self.bi_avalues = self._create_star_routing_embed_with_p(
