...
class BasicDataset(Dataset):
    ...
    def load_U_I_interaction(self):
        with open(os.path.join(self.path, self.name, 'user_item.txt'), 'r') as f:
            return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
    def load_B_I_affiliation(self, train=True):
        RHO_MASK_MAP = {
            -4: '0.9',
            -2: '0.7',
            0: '0.5',
        ...
        if train:
            ...
            with open(os.path.join(self.path, self.name,
                                'BPRMF_bundle_item_mask0.5.txt'), 'r') as f:
                return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')),
                                f.readlines()))
        else:
            rho   = CONFIG.get('rho', 0)
...
class AssistDataset(BasicDataset):
    def __init__(self, path, name):
        ...
        # B-I tune&test
        self.B_I_pairs = self.load_B_I_affiliation(train=False)
        indice = np.array(self.B_I_pairs, dtype=np.int32)
        values = np.ones(len(self.B_I_pairs), dtype=np.float32)
        self.ground_truth_b_i_test = sp.coo_matrix(
            (values, (indice[:, 0], indice[:, 1])), shape=(self.num_bundles, self.num_items)).tocsr()

        print_statistics(self.ground_truth_b_i,'B-I statistics train')
        print_statistics(self.ground_truth_b_i_test, 'B-I statistics test')


def get_dataset(path, name, task='tune', seed=123):  #######  1.train 2.tune test
