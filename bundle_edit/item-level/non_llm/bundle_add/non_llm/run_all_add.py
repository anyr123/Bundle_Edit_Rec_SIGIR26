'''
Description: 批量运行所有add任务模型并生成对比报告
Author: anyiran
Date: 2025-09-21
Usage: python run_all_add.py -d clothing
'''
...
def run_model(model_name, dataset):
    ...
    if not os.path.exists(model_file):
        print(f"[ERROR] 找不到脚本文件 {model_file}")
        return False, None
    
    cmd = ['python', model_file, '-d', dataset]
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"[SUCCESS] {model_name.upper()} 运行成功！耗时: {(end_time - start_time)/60:.2f} 分钟")
            return True, (end_time - start_time) / 60
        else:
            print(f"[ERROR] {model_name.upper()} 运行失败！")
            print(f"错误信息: {result.stderr}")
            return False, None
            
    except Exception as e:
        print(f"[ERROR] 运行 {model_name.upper()} 时发生错误: {e}")
        return False, None
...
def generate_comparison_report(dataset, results):
    ...
    print("\n" + "="*80)
    print(f"[REPORT] {dataset.upper()} 数据集 - Add任务模型对比报告")
    print("="*80)
    ...
    # 保存详细报告到文件
    report_file = f'../result/{dataset}/add_comparison_report.txt'
    os.makedirs(f'../result/{dataset}', exist_ok=True)
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write(f"Bundle Add Task - 模型对比报告\n")
...
def main():
    parser = argparse.ArgumentParser(description='批量运行所有add任务模型')
    ...
    parser.add_argument('-m', '--models', type=str, nargs='+',
                        ...
                        default=['tsf', 'mean_vae', 'itemknn', 'concat_vae', 'bprmf'],
    ...
    # 结果文件映射
    result_file_mapping = {
        'tsf': f'../result/{args.dataset}/tsf_add_{args.dataset}_results.txt',
        'mean_vae': f'../result/{args.dataset}/mean_vae_add_{args.dataset}_results.txt',
        'itemknn': f'../result/{args.dataset}/itemknn_add_{args.dataset}_results.txt',
        'concat_vae': f'../result/{args.dataset}/concat_vae_add_{args.dataset}_results.txt',
        'bprmf': f'../result/{args.dataset}/bprmf_add_{args.dataset}_results.txt'
    }
    ...
    for model in args.models:
        ...
        # 运行模型
        success, time_cost = run_model(model, args.dataset)
