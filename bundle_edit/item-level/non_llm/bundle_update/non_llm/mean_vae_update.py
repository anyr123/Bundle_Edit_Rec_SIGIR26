'''
Description: Mean-VAE模型用于bundle update任务 - 包含模型训练+验证
基于bundle add任务的完整实现，结合delete和add功能
Author: anyiran
Date: 2025-09-21
LastEditTime: 2025-09-21 16:35:09
'''
...
# -------------------------------
def calculate_item_anomaly_score_meanvae(bundle_items, item_embeddings, item2id):
    """
    使用Mean-VAE embeddings计算bundle中每个item的异常分数
    方法：基于K最近邻距离 + 随机噪声
...
# -------------------------------
def select_best_candidate_meanvae(bundle_items, candidate_ids, item_embeddings, item2id, id2item):
    """
    使用Mean-VAE embeddings从候选集中选择最适合bundle的item
...
for idx, (bundle_id, bundle_items, candidate_ids) in enumerate(test_data):
    ...
    try:
        ...
        # Phase 1: 使用Mean-VAE Delete检测离群item
        predicted_remove = None
        if len(bundle_items) > 1 and items_to_remove:
            anomaly_scores = calculate_item_anomaly_score_meanvae(bundle_items, item_embeddings, item2id)
            if anomaly_scores:
                predicted_remove = max(anomaly_scores.keys(), key=lambda x: anomaly_scores[x])
        
        # Phase 2: 使用Mean-VAE Add选择最佳候选item
        ...
        predicted_add = select_best_candidate_meanvae(updated_bundle, candidate_ids, item_embeddings, item2id, id2item)
...
# -------------------------------
print("\n" + "="*80)
print("Mean-VAE Bundle Update任务验证结果")
print("="*80)
...
if successful_count > 0:
    ...
    result_text = f"""Mean-VAE Update Task Results - {args.d}
Dataset: {args.d}
...
- Total items: {num_items}"""
    ...
    # 保存结果到文件
    result_file = os.path.join(result_dir, f'mean_vae_update_{args.d}_results.txt')
    with open(result_file, 'w') as f:
        f.write(result_text)
    
    # # 保存详细结果
    # detailed_results_file = os.path.join(result_dir, f'mean_vae_update_{args.d}_detailed.txt')
...
print("模型: 单一Mean-VAE模型同时处理delete和add任务")