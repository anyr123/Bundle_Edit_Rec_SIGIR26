'''
Description: Process bundle operation test data using In-Context Learning LLM with YAML configuration
Author: anyiran
Date: 2025-07-13
'''
import pandas as pd
...
def load_fewshot_config(config_path='../props/fewshot.yaml'):
    """
    Load In-Context Learning configuration from YAML file.
    
    Args:
        config_path (str): Path to the YAML configuration file
        
    Returns:
        dict: Configuration dictionary containing FEWSHOT prompt templates
    """
    try:
...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, default='../props/fewshot.yaml', 
                        help='Path to In-Context Learning configuration YAML file')
    
    args = parser.parse_args()

    # Load In-Context Learning configuration
    ...
    for _, row in test_data.iterrows():
        ...
        if test_type == 'add':
            ...
            # For DELETE: 随机打乱bundle items的顺序，让随机添加的item位置随机化
            bundle_items_shuffled = bundle_items.copy()
            random.shuffle(bundle_items_shuffled)
            
            bundle_lines = ',\n'.join([f'{idx}. "{item[1]}"' for idx, item in enumerate(bundle_items_shuffled)])
            ...
            if items_to_delete:
                # 在随机排序后的bundle中找到应该被删除的item位置
                for idx, (item_id, title) in enumerate(bundle_items_shuffled):
                    if item_id in items_to_delete:
                        test_cases.append({
                            'test_type': test_type,
                            'bundle_id': bundle_id,
                            'gt_pos': str(idx),
                            'bundle_items': bundle_items_shuffled,  # 使用随机排序后的items
                            'candidate_items': []
                        })
                        break
            else:
    ...
    # Save results
    output_file = os.path.join(save_data_path, f'{args.mode}_fewshot_results.txt')
