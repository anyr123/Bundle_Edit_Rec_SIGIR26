...
def load_cot_config(config_path='../props/cot.yaml'):
    """
    Load Chain of Thought configuration from YAML file.
    
    Args:
        config_path (str): Path to the YAML configuration file
        
    Returns:
        dict: Configuration dictionary containing CoT prompt templates
    """
    try:
...
def get_bundle_topic(bundle_lines, max_retries=5):
    """
    Get bundle topic using GPT API
    """
    topic_prompt = f"""
...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, default='../props/cot.yaml', 
                        help='Path to Chain of Thought configuration YAML file')
    
    args = parser.parse_args()

    # Load Chain of Thought configuration
    config = load_cot_config(args.config)
    if config is None:
        return
    ...
    print("Generating bundle topics...")
    for _, row in test_data.iterrows():
        ...
        if test_type == 'add':
            ...
            # For DELETE: 当前bundle包含应该被删除的item
        ...
        # 为每个有效的测试案例生成bundle topic
        if test_cases and test_cases[-1]['bundle_id'] == bundle_id:
            # 使用当前test case中的bundle_items来构建bundle_lines
            current_bundle_items = test_cases[-1]['bundle_items']
            bundle_lines = ',\n    '.join([f'{idx}. "{item[1]}"' for idx, item in enumerate(current_bundle_items)])
            
            # 调用 GPT 获取 topic
            topic_text = get_bundle_topic(bundle_lines)
            bundle_topics[bundle_id] = topic_text
            print(f"[Bundle {bundle_id}] Topic: {topic_text}")
    ...
    for test_case in test_cases:
        ...
        if test_case['test_type'] in ['add', 'update']:
            cand_lines = ',\n    '.join([f'{i}. "{it[1]}"' for i, it in enumerate(candidate_items)])
        else:
            cand_lines = ""

        # Build prompt from config with topic
        prompt_template = config[prompt_key]
        topic = bundle_topics.get(bundle_id, "Unknown topic")
    ...
    # Save results
    output_file = os.path.join(save_data_path, f'{args.mode}_cot_results.txt')
