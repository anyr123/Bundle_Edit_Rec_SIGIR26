...
class PromptGenerator(object):  
        ...
        if type == 'bundle':
            ...
            for error in sorted(error_dict.keys()):
                if len(error_dict[error]) > 1:
                    feed_tips += ' and '.join(map(str, error_dict[error])) +' are '+ error_types[error]
                else:
                    print(error_dict)
                    feed_tips += str(error_dict[error][0]) +' is '+ error_types[error]
    ...
    def get_test_prompts(self, data_info):
        ...
        return test_prompts
    #Zero-shot directly adopts LLMs to generate bundles and infer intents from user sessions.
    def get_zero_shot_prompts(self, session_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed."""
        ...
        return Template(Init_generated_bundles).substitute(session_info=session_info)
    #(1) Few-shot-random randomly selecting different examples for each test session as the demonstration
    def get_few_shot_random_prompts(self, session_info,example_session,example_bundle_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed.
    For example:
    A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $example_session. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed.
    $example_bundle_info
    """

        return Template(Init_generated_bundles).substitute(session_info=session_info,example_session=example_session,example_bundle_info=example_bundle_info)
    #(2) Few-shot-fix randomly selecting the same examples, and use them as demonstrations for all test sessions
    def get_few_shot_fix_prompts(self, session_info,example_session,example_bundle_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed.
    ...
    """
        ...
        return Template(Init_generated_bundles).substitute(session_info=session_info,example_session=example_session,example_bundle_info=example_bundle_info)
    #(3) Few-shot-top using the nearest neighbor sessions (same as in AICL) for each test session as the demonstration.
    def get_few_shot_top_prompts(self, session_info,example_session,example_bundle_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed.
    ...
    def get_zero_shot_rectest_1_prompts(self, session_info,candidate_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product number']}.
    No explanation for the results is needed."""

        return Template(Init_generated_bundles).substitute(session_info=session_info,candidate_info=candidate_info)
    
        
    def get_zero_shot_rectest_2_prompts(self, user_bundle,candidate_text_order): 
        Init_generated_bundles = f"A bundle can be a set of alternative or complementary products that are purchased with a certain intent. According to the my purchasing records generate the corresponding bundle records:\n{user_bundle}\n\n" \
                    f"Now there are 20 candidate bundles of clothing that I can buy next:\n{candidate_text_order}\n" \
                    f"Please rank these 20 bundles by measuring the possibilities that I would like to buy next most, according to bundle history. Let's think step by step\n" \
                    f"Please show me your ranking results with order numbers. Split your output with line break. You MUST rank the given candidate bundles. You can not generate bundles that are not in the given candidate list."

        return Template(Init_generated_bundles).substitute(user_bundle=user_bundle,candidate_text_order=candidate_text_order)
    
    def get_zero_shot_prompts_user(self, session_info):
        Init_generated_bundles = """A bundle can be a set of alternative or complementary products that are purchased with a certain intent. 
    Please detect bundles from a sequence of products. Each bundle must contain multiple products. 
    Here are the products and descriptions: $session_info. The answer format is: {'bundle number':['product']}.
    No explanation for the results is needed."""
