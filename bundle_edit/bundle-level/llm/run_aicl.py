...
if __name__ == '__main__':
    ...
    for test_id, (topk_session_idx, bundle_dict) in tqdm(parsered_res.items()):
        ...
        for _ in range(N_iter):
            print(type(bundle_dict))
            if not isinstance(bundle_dict, dict):
                if isinstance(bundle_dict, tuple):
                    if len(bundle_dict) >= 2:
                        bundle_dict={**bundle_dict[0],**bundle_dict[1]}
                    else:
                        bundle_dict=bundle_dict[0]
    ...
    for test_id, (topk_session_idx, context) in intent_context.items():
        ...
        print("bundle_res:")
        print(output_parser(context[-3]['content']))
        print("intent_res:")
        print(output_parser(context[-1]['content'], type='intent'))
        intent_res = output_parser(context[-1]['content'], type='intent')
        ...
        if bundle_res['state_code'] == 404:
            ...
            if not isinstance(intent_dict, dict):
                if isinstance(intent_dict, tuple):
                    if len(intent_dict) >= 2:
                        intent_dict={**intent_dict[0],**intent_dict[1]}
                    else:
                        intent_dict=intent_dict[0]
            for bundle_id, items in bundle_dict.items():
                ...
                for gdbundle in ground_truth_bundles:
                    ...
                    if reidx_items <= bundle_list:
                        if 'bundle' in str(bundle_id) and len(bundle_id) < 10:
                            try:
                                related_bundles.append((','.join(list(reidx_items)), intent_dict[bundle_id], gdbundle[-1], gdbundle[0]))
                            except KeyError as e:
                                #
                                # keys1 = list(intent_dict.keys())
                                # values1 = list(intent_dict.values())
                                
                                # keys2 = list(bundle_dict.keys())
                                
                                # # 检查 dict2 的键是否比 dict1 的多或少
                                # if len(keys2) != len(keys1):
                                print("Error: The number of keys in both dictionaries must be the same.")
                                    
                                
                                # 创建新的字典，使用 dict2 的键，dict1 的值
                                # new_dict = dict(zip(keys2, values1))
                        else: # intent:bundle
    ...
    intent_feedback_res = {}
    intent_rater_models = config.get('rater', [])
    intent_raters = []
            ...
            for _ in range(3):
                ...
                for idx, (bid, intent) in enumerate(intent_res.items()):
                    ...
                    try:
                        ...
                        #'3,'
                        #i="intent1",提取数字
                        intent[key_list[1]] = [num for item in intent[key_list[1]] for num in re.findall(r'\d+', item)]             
                        scores_res[idx][1] += np.array([int(i.replace(",","")) for i in intent[key_list[1]]]) #intent 2
                    try:
                        scores_res[idx][1] += np.array([int(i) for i in intent[key_list[1]]]) #intent 2
                    except IndexError as e:
                        #key_list[1] IndexError: list index out of range
                        #没有intent2
                        print(f"key_list[1] caused IndexError: {e}")
                        print(f"scores_res[idx][1] before exception: {scores_res[idx][1]}")
                    except ValueError as e:
                        #'3,'
                        #i="intent1",提取数字
                        intent[key_list[1]] = [num for item in intent[key_list[1]] for num in re.findall(r'\d+', item)]                
